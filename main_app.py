...
def translate_text_openai(text, source_language, target_language, api_key):
    ...
    try:
        response = openai.ChatCompletion.create(
            model="gpt-4o",  # Bitte hier das Modell anpassen, falls benötigt.
            messages=[
...
# ------------------------------------------------------------------
# 7) Excel Online Search - Placeholder (falls benötigt)
# ------------------------------------------------------------------
...
"""
    ...
    # Mehrere Excel-Dateien erstellen und in st.session_state["excel_files"] sammeln
    if uploaded_files and api_key:
        if st.button("Alle Analysen durchführen & in Excel speichern (Multi)"):
            ...
            for fpdf in selected_files_for_excel:
                ...
                try:
                    ...
                    return
                ws = wb.active
                # Fülle Excel-Zellen: D2 = Hauptthema, D5 = Genname, D6 = rs-Nummer, etc.
                ...
                ws["G21"].value = summary_result.split("\n")[0]  # Beispiel: erste Zeile als "Results"
                ws["G22"].value = summary_result.split("\n")[-1]  # Beispiel: letzte Zeile als "Conclusion"
                # J2: aktuelles Datum
                ...
                # Excel-Dateien für das Merging sammeln
                if "excel_files" not in st.session_state:
                    st.session_state["excel_files"] = []
    ...
    # Button zum Zusammenführen aller erstellten Excel-Dateien
...
# ------------------------------------------------------------------
# Sidebar Navigation und Chatbot (Chatbot in rechter Sidebar, scrollbar)
# ------------------------------------------------------------------
