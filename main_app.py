...
def split_summary(summary_text):
    """
    Versucht 'Ergebnisse' und 'Schlussfolgerungen' (bzw. 'Fazit') zu splitten.
    Diese Funktion ist beispielhaft und versucht eine naive Trennung.
    """
    # Du kannst hier die Muster anpassen, falls du andere Überschriften verwendest.
    # Beispielsweise: 'Ergebnisse' / 'Ergebnis' / 'Resultate' usw.
    # Gleiches für 'Schlussfolgerungen' / 'Fazit'.
    pattern = re.compile(
    ...
    if match:
        ...
        # Falls keine klaren Abschnitte gefunden werden
        return summary_text, ""
...
"""
        try:
            ...
            try:
                ...
                # Falls kein korrektes JSON, packen wir einfach alles in eine Liste
...
"""
    ...
    if uploaded_files and api_key:
        if st.button("Alle Analysen durchführen & in Excel speichern (Multi)"):
            with st.spinner("Analysiere alle hochgeladenen PDFs (für Excel)..."):
                ...
                # Wenn Compare Mode aktiv ist, nutzen wir ggf. die "relevanten Paper" aus dem Outlier-Check
                if compare_mode:
                ...
                for fpdf in selected_files_for_excel:
                    ...
                    # Wir erstellen standardmäßig immer eine deutsche Zusammenfassung:
                    ...
                    # Falls kein direkter match => Excel-Liste prüfen (Beispielhaft)
                    if not gene_via_text:
                        try:
                            ...
                            # Falls die Datei nicht existiert, einfach nichts machen
                            pass
                    ...
                    # parse_cohort_info und split_summary auf die deutsche Zusammenfassung anwenden
                    ...
                    # Nun in vorlage_paperqa2.xlsx schreiben
                    try:
                    ...
                    # Hier befüllen wir nun die gewünschten Zellen:

                    # D2: Hauptthema (aus st.session_state["theme_compare"], sonst "N/A")
                    ws["D2"].value = st.session_state.get("theme_compare", "N/A")

                    # D5: Gen (aus found_gene)
                    ...
                    # E10–E12: jeweils freq_info
                    # Falls es weniger als 3 Genotypen gibt, Zellen ggf. leer lassen
                    for i in range(3):
                        row_index = 10 + i
                    ...
                    # D20: Studiengröße + Ethnie (zusammengeführt aus parse_cohort_info)
                    ws["D20"].value = cohort_info

                    # E20: Key Findings
                    ws["E20"].value = key_findings_result

                    # G21: Results (erster Teil der Zusammenfassung)
                    ws["G21"].value = ergebnisse

                    # G22: Conclusion (zweiter Teil der Zusammenfassung)
                    ws["G22"].value = schlussfolgerungen

                    # Optional: Relevanz, Methoden usw. könnten in weitere Zellen geschrieben werden.
                    # Beispielhaft, falls du das willst (nicht angefordert, daher nur als Beispiel):
                    # ws["E22"].value = methods_result
                    # oder ws["E23"].value = relevance_result

                    # Arbeitsmappe in Memory speichern
                    output_buffer = io.BytesIO()
                    wb.save(output_buffer)
                    output_buffer.seek(0)
                    
                    # Download-Button
                    file_name_excel = f"analysis_{fpdf.name.replace('.pdf', '')}.xlsx"
                    st.download_button(
    ...
    # --- Abschnitt: PaperQA Multi-Paper Analyzer (Gemeinsamkeiten & Widersprüche) ---
    st.write("---")
    st.header("PaperQA Multi-Paper Analyzer: Gemeinsamkeiten & Widersprüche")
    
    # Auswahl der Quelle für die Analyse:
    ...
    if analysis_source_choice == "Gescorte Paper":
        ...
        if st.button("Analyse (Hochgeladene Paper) durchführen"):
            if "paper_texts" not in st.session_state or not st.session_state["paper_texts"]:
                # Falls wir noch keine Texte haben, aus den hochgeladenen extrahieren
                if uploaded_files:
                    st.session_state["paper_texts"] = {}
                    ...
                    st.error("Keine hochgeladenen Paper vorhanden.")
                    return
